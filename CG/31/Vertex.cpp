...
Vertex::Vertex(const std::vector<float>& verts, const std::vector<unsigned int>& numVerts)
	: numVerts(static_cast<unsigned int>(verts.size() / 8)), numIndices(static_cast<unsigned int>(numVerts.size()))
{
	glGenVertexArrays(1, &VAO);
	glBindVertexArray(VAO);
	...
	glGenBuffers(1, &indexBuffer);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBuffer);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<unsigned int>(numVerts.size()) * sizeof(unsigned int), numVerts.data(), GL_STATIC_DRAW);
...
Vertex::Vertex(const std::vector<float>& verts)
	:numVerts(static_cast<unsigned int>(verts.size() / 8)), indexBuffer(0), numIndices(0)
{
	glGenVertexArrays(1, &VAO);
	glBindVertexArray(VAO);
	...
	glGenBuffers(1, &VBO);
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	glBufferData(GL_ARRAY_BUFFER, static_cast<unsigned int>(verts.size()) * sizeof(verts[0]), verts.data(), GL_STATIC_DRAW);
...
Vertex::Vertex(const std::vector<glm::vec3>& verts)
	:numVerts(static_cast<unsigned int>(verts.size() / 3)), indexBuffer(0), numIndices(0)
{
	glGenVertexArrays(1, &VAO);
	glBindVertexArray(VAO);
